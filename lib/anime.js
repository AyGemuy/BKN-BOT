    ...
    axios
      ...
      .then((get) => {
        ...
        axios.get(random + "/pics").then((res) => {
          ...
          let hasil = [];
          $$(
            "#content > table > tbody > tr > td:nth-child(2) > table > tbody > tr"
          ).each(function (a, b) {
            hasil.push({
...
async function fandom(q) {
  return new Promise((resolve, reject) => {
    axios
      .get(
        `https://jump.fandom.com/wiki/Special:Search?query=${q}&scope=internal&navigationSearch=true`
      )
      .then((get) => {
        let $ = cheerio.load(get.data);
        let getres = [];
        $(
          "#mw-content-text > section > div > div.unified-search__layout__main > ul > li"
        ).each(function (a, b) {
        ...
        axios.get(random).then((res) => {
          ...
          let info = [];
          $$("#mw-content-text > div > table > tbody > tr").each(function (
            a,
            b
          ) {
            info.push({
          ...
          let hasil = {
            ...
            image:
              $$(
                "#mw-content-text > div > table > tbody > tr:nth-child(1) > td > a"
              ).attr("href") ||
              $$(
                "#mw-content-text > div > table > tbody > tr:nth-child(1) > td > a > img"
              ).attr("src"),
            part,
            info,
            lisensi: $$(
