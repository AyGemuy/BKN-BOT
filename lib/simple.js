    ...
    reply: {
      ...
      value(jid, text = "", quoted, options) {
        return Buffer.isBuffer(text)
          ...
          : conn.sendMessage(
              jid,
              { text, contextInfo: global.adReply?.contextInfo, ...options },
              { quoted, mentions: conn.parseMention(text), ...options }
            );
      },
    },
    sendOrder: {
      async value(jid, buffer, harga, item, title, text, qu) {
...
END:VCARD`.trim();
    ...
    sendLocUrl: {
      async value(jid, text, fot, urll, buffer, quoted, options = {}) {
        let file = await conn.resize(buffer, 300, 150);
