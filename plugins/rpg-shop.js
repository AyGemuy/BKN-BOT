...
`.trim();
  ...
  if (command.toLowerCase() == "buy") {
    ...
    if (user[paymentMethod] < listItems[item][paymentMethod] * total)
      return m.reply(
        `Anda tidak memiliki cukup ${global.rpg.emoticon(
          paymentMethod
        )}${paymentMethod} untuk membeli *${total}* ${global.rpg.emoticon(
          item
        )}${item}. Anda membutuhkan *${
          listItems[item][paymentMethod] * total - user[paymentMethod]
        }* lagi ${paymentMethod} untuk dapat membeli`
      );
    user[paymentMethod] -= listItems[item][paymentMethod] * total;
    user[item] += total;
    return m.reply(
      `Anda membeli *${total}* ${global.rpg.emoticon(item)}${item}`
    );
  } else {
    if (user[item] < total)
      return m.reply(
    ...
    return m.reply(
      `Anda menjual *${total}* ${global.rpg.emoticon(item)}${item}`
    );
  }
};
