...
let handler = async (m, { conn, args }) => {
  ...
  const items = Object.keys(inventory.items)
    .map(
      (v) => user[v] && `${global.rpg.emoticon(v)} ${v}: ${NumberF(user[v])}`
    )
    .filter((v) => v)
    .join("\n")
    .trim();
  ...
  const cooldowns = Object.entries(inventory.cooldowns)
    ...
    .join("\n")
    .trim();
  const caption = `🧑🏻‍🏫  ɴᴀᴍᴇ: ${conn.getName(who)}
...
*───── ᴘᴇᴛs ─────*
${pets ? pets + "\n" : ""}${
          user.petFood ? "🍖 ᴘᴇᴛғᴏᴏᴅ: " + NumberF(user.petFood) : ""
        }`
      : ""
  }${
    cooldowns
