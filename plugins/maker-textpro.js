...
let handler = async (
  m,
  { conn, args: [effect], text: txt, usedPrefix, command }
) => {
  if (!effect)
    throw (
  ...
  if (!effects.find((v) => new RegExp(v.title, "gi").test(effect)))
    throw `Efek *${effect}* tidak ditemukan`;
  ...
  if (typeof res == "number")
    throw res == -1
      ? `Efek *${effect}* tidak ditemukan`
      : `Gunakan format ${usedPrefix}${command} ${effect} ${new Array(res)
          .fill("text")
          .map((v, i) => v + (i ? i + 1 : ""))
          .join("|")}`;
