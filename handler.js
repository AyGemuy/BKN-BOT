...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    if (opts["pconly"] && !isOwner && m.chat.endsWith("g.us")) return;
    if (
      opts["gconly"] &&
      !isOwner &&
      !isPrems &&
      ...
      return conn.reply(
        m.chat,
        `Maaf, Bot ini sedang mode Group Only artinya hanya aktif jika di group.\nJika ingin menggunakan atau memasukkan bot ke dalam group maka chat owner wa.me/${nomorown} untuk memberikan akses.`,
        fkontak
      );
    ...
    try {
      ...
      if (user) {
        ...
        if (!user.registered) {
          if (!("name" in user)) {
            user.name = m.name;
          } else user.name = m.name;
          if (!isNumber(user.age)) user.age = -1;
          if (!isNumber(user.regTime)) user.regTime = -1;
        }
        if (!("username" in user)) {
          user.username = m.name;
        } else user.username = m.name;
      ...
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          try {
            ...
            let flar = action == "add" ? thm : thg;
            let wecav = action == "add" ? wel : lea;
            conn.sendFile(
              id,
              await (
                await conn.getFile(wecav)
              ).data,
              "",
              `${text}\n${botdate}`,
              null,
              false,
              {
                contextInfo: {
                  ...
                  externalAdReply: {
                    ...
                    title: "Hᴀʟᴏ Nɢᴀʙ",
                    body: wm,
                    thumbnail: await (await conn.getFile(flar)).data,
                    thumbnailUrl: flar,
                    sourceUrl: sgc,
                  },
                },
...
global.dfail = (type, m, conn) => {
  ...
  let msg = {
    ...
    premium:
      "[ PREMIUM USER ONLY ]\n\n🚩 Menggunakan fitur ini di private chat hanya untuk pengguna premium, upgrade ke paket premium hanya 10k,- untuk mendapatkan batas 1K selama 1 bulan.\n\nJika anda ingin membeli paket premium silahkan kirim *.sewabot*",
