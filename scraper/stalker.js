...
async function twitterStalk(user) {
  return new Promise((resolve, reject) => {
    axios.get("https://instalker.org/" + user).then(({ data }) => {
      ...
      $("div.activity-posts").each(function (a, b) {
        tweets.push({
          author: {
            ...
            profile_pic:
              $(b).find("img").attr("src") || $(b).find("img").attr("onerror"),
  ...
  return new Promise((resolve, reject) => {
    axios({
      ...
      headers: {
        ...
        cookie:
          "crsftoken=au1sbQzCDp2Ri5TZF688kVeWOfvaa3Ox; ds_user_id=26641471282; ig_did=54AA06C9-3A86-4F53-A7F6-C9127DC153AC; ig_nrcb=1; mid=YU0dFwALAAHPggeAvlvEiV8DP9zM; sessionid=26641471282%3AnPDRQn9Qe628rm%3A26",
      },
    }).then((data) => {
      const Format = {
    ...
    axios({
      ...
      headers: {
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36",
        cookie:
          "_ga=GA1.2.2145921501.1640416771; _gat=1; _gid=GA1.2.1701284558.1640416771; _ym_d=1640416774; _yn_uid=1640416774851926759; tgstat_sirk=se5opc311gme57h03vl7q72r44",
      },
    }).then(({ data }) => {
      let $ = cheerio.load(data);
      let hasil = {
        ...
        username: $(
          "div.small-12.medium-5.large-5.columns > a:nth-child(3) > span:nth-child(2)"
        ).text(),
        channel_url: $(
          "div.small-12.medium-5.large-5.columns > a:nth-child(3)"
        ).attr("href"),
        likes: $(
          "div.small-12.medium-5.large-5.columns > a.btn.btn-default.btn-sm.btn-heart.not-hover.popup_ajax > span"
        )
          .text()
          .replace("Like", ""),
        description: $(
          "div.small-12.medium-5.large-5.columns > div:nth-child(8)"
        )
          .text()
          .trim(),
        channel_geo_lang: $(
          "div.medium-4.large-4.columns.hide-for-small-only > div:nth-child(1) > div:nth-child(1)"
        )
          .text()
          .replace(`Channel's geo & Language`, "")
          .trim(),
        channel_category: $(
          "div.medium-4.large-4.columns.hide-for-small-only > div:nth-child(1) > div:nth-child(2)"
        )
          .text()
          .replace("Category", "")
          .trim(),
        members: $(
          "body > div.wrap > div > div.row.margin-top15.small-no-padding > div:nth-child(1) > div > div:nth-child(1)"
        )
          .text()
          .trim(),
        post_reach: $(
          "body > div.wrap > div > div.row.margin-top15.small-no-padding > div:nth-child(2) > div > div:nth-child(1)"
        )
          .text()
          .trim(),
        daily_reach: $(
          "body > div.wrap > div > div.row.margin-top15.small-no-padding > div:nth-child(3) > div > div:nth-child(1)"
        )
          .text()
          .trim(),
        post_per_day: $(
          "body > div.wrap > div > div.row.margin-top15.small-no-padding > div:nth-child(4) > div > div:nth-child(1)"
        )
          .text()
          .trim(),
        err: $(
          "body > div.wrap > div > div.row.margin-top15.small-no-padding > div:nth-child(5) > div > div:nth-child(1)"
        ).text(),
        citation_index: $(
          "body > div.wrap > div > div.row.margin-top15.small-no-padding > div:nth-child(6) > div > div:nth-child(1)"
        ).text(),
        mentions_of_channel: $(
          "div.columns.large-4.medium-12.small-12 > b"
        ).text(),
        ...
        profile_pic:
          "https:" +
            $(
              "body > div.wrap > div > div.row.small-collapse.medium-uncollapse.sticky-canceller-container.margin-bottom15 > div.small-12.medium-3.large-3.columns > div > img"
            ).attr("data-src") ||
